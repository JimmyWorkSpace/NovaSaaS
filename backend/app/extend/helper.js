...
module.exports = {
  ...
  getDB(ctx, forWrite = false) {
    ...
    const { driver, instance } = readWriteSplit && !forWrite ? slave : master;
    const dbLayer = ctx.service.db;
    if (!dbLayer) {
      throw new Error('ctx.service.db 未就绪，请确认 app/service/db 下的服务已正确加载');
    }
    const driverLayer = dbLayer[driver];
    if (!driverLayer) {
      throw new Error(`ctx.service.db.${driver} 未定义，请确认 config.database.driver 为 "${driver}" 且对应插件已启用`);
    }
    const instanceLayer = driverLayer[instance];
    if (!instanceLayer) {
      throw new Error(`ctx.service.db.${driver}.${instance} 未定义，请确认 config.database.instance 与数据库配置中的 clients 键名一致`);
    }
    return /** @type {import('egg').IService['db']['mysql']['ruoyi']} */ (instanceLayer);
  },
